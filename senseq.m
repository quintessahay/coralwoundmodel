...
function [sens, y, sol] = senseq(pars,x,Init,set,ydata)

[sol,y] = model_sol(pars,x,Init,set,ydata);

%Depending on form of y, the sensitivity matrix could be dymodel/dpars or 
%dr/dpars where r = ymodel - ydata

[sens] = diffjac(pars,@(pars,x,Init) myfun(pars,x,Init,set,ydata),y,x,Init);


function y = myfun(pars,x,Init,set,ydata)

[~,y] = model_sol(pars,x,Init,set,ydata);
